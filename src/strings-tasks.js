...
function concatenateStrings(value1, value2) {
  return String(value1 || '').concat(value2 || '');
}
...
function isPalindrome(str) {
  const check = /[^A-Za-z0–9]/g;
  return (
    str.toLowerCase().replace(check, '') ===
    str.toLowerCase().replace(check, '').split('').reverse().join('')
  );
}
...
function findLongestWord(sentence) {
  return sentence
    ...
    .reduce(
      (acc, currentValue) =>
        currentValue.length > acc.length ? currentValue : acc,
      ''
    );
}
